...
plot(output$InfHumans, output$ResInfHumans, xlim = c(-0.001,1))
...
#### Model Testbed#####
#For Particular Parameter Combinations#
...
parms4 = c(ra = 0.01, rh =  0.5, ua = 0.01, uh = 0.001, betaAA = 0.05, betaAH = 0.01, betaHH = 0.01, 
           betaHA = 0.01, phi = 0.95, tau = 0.05, theta = 0.5)

out <- ode(y = init, func = amr, times = times1, parms = parms4)
...
#### Comb IRH and IH Measure Testing - Effect of Treatment and Surface Plots + Theta and Other Parameters ####

# Testing for the Effect of Treatment on a Combined Measure
...
#### Change in Icomb* ####
betaHArange <- seq(0,1, by=0.001)
betaAArange <- seq(0,1, by=0.001)
tauchange <- data.frame()

for (i in 1:length(betaHArange)) {
  temp <- data.frame(matrix(NA, nrow = 1, ncol=4))
  parms1 = c(ra = 0.01, rh =  0.01, ua = 0.01, uh = 0.001, betaAA = betaAArange[i], betaAH = 0.001, betaHH = 0.001, 
             betaHA = 0.001,  phi = 0.05, kappa = 0.05, nu = 0.05, tau = 0, theta = 0.2)
  parms2 = c(ra = 0.01, rh =  0.01, ua = 0.01, uh = 0.001, betaAA = betaAArange[i], betaAH = 0.001, betaHH = 0.001, 
             betaHA = 0.001, phi = 0.05, kappa = 0.05, nu = 0.05, tau = 1, theta = 0.2)
  out <- ode(y = init, func = amr, times = times, parms = parms1)
  out1 <- ode(y = init, func = amr, times = times, parms = parms2)
  temp[1,1] <- betaHArange[i]
  temp[1,2] <- round(out[nrow(out),6],digits = 5) + round(out[nrow(out),7], digits = 5)
  temp[1,3] <- round(out1[nrow(out),6], digits = 5) + round(out1[nrow(out),7], digits = 5)
  temp[1,4] <- (((temp[1,2])/(temp[1,3])) - 1)*100
  print(temp[1,4])
  tauchange <- rbind.data.frame(tauchange, temp)
}

for (i in 1:length(betaHArange)) {
  temp <- data.frame(matrix(NA, nrow = 1, ncol=4))
  parms1 = c(ra = 0.01, rh =  0.01, ua = 0.01, uh = 0.001, betaAA = 0.01, betaAH = 0.001, betaHH = 0.001, 
             betaHA = betaHArange[i], phi = 0.05, kappa = 0.05, nu = 0.05, tau = 0, theta = 0.2)
  parms2 = c(ra = 0.01, rh =  0.01, ua = 0.01, uh = 0.001, betaAA = 0.01, betaAH = 0.001, betaHH = 0.001, 
             betaHA = betaHArange[i], phi = 0.005, kappa = 0.005, nu = 0.005, tau = 1, theta = 0.2)
  out <- ode(y = init, func = amr, times = times, parms = parms1)
  out1 <- ode(y = init, func = amr, times = times, parms = parms2)
  temp[1,1] <- betaHArange[i]
  temp[1,2] <- round(out[nrow(out),6],digits = 6) + round(out[nrow(out),7], digits = 6)
  temp[1,3] <- round(out1[nrow(out),6], digits = 6) + round(out1[nrow(out),7], digits = 6)
  temp[1,4] <- (((temp[1,2])/(temp[1,3])) - 1)*100
  print(temp[1,4])
  tauchange <- rbind.data.frame(tauchange, temp)
}

surfaceoutputplot$new <- round(surfaceoutputplot$`Comb Inf Res/Sens Humans`, digits = 5)

colnames(tauchange)[1:4] <- c("BetaHA","ICombTau0","ICombTau1","ChangeIComb")
plot(tauchange$BetaHA, tauchange$ChangeIComb, type = "l", xlab = "BetaHA",
     ylab = "% Increase in IComb* - Antibiotic Curtailment", lwd=2)

#### ####

betaAArange <- seq(0,1, by=0.01)
betaHArange <- seq(0,1, by=0.01)

tauchange <- data.frame()

combparmtau <- expand.grid(betaAArange, betaHArange)
colnames(combparmtau)[1:2] <- c("betaAArange","betaHArange")


init <- c(Sa=0.99, Ia=0.01, Ira=0, Sh=1, Ih=0, Irh=0)
times <- c(0,9999,10000) 

for (i in 1:nrow(combparmtau)) {
  temp <- data.frame(matrix(NA, nrow = 1, ncol=5))
  parms1 = c(ra = 0.01, rh =  0.01, ua = 0.01, uh = 0.001, betaAA = combparmtau[i,1], betaAH = 0.001, betaHH = 0.001, 
             betaHA = combparmtau[i,2], phi = 0.05, kappa = 0.05, nu = 0.05, tau = 0, theta = 0.2)
  parms2 = c(ra = 0.01, rh =  0.01, ua = 0.01, uh = 0.001, betaAA = combparmtau[i,1], betaAH = 0.001, betaHH = 0.001, 
             betaHA = combparmtau[i,2], phi = 0.05, kappa = 0.05, nu = 0.05, tau = 1, theta = 0.2)
  out <- ode(y = init, func = amr, times = times, parms = parms1)
  out1 <- ode(y = init, func = amr, times = times, parms = parms2)
  temp[1,1] <- combparmtau[i,1]
  temp[1,2] <- combparmtau[i,2]
  temp[1,3] <- out[nrow(out),6] + out[nrow(out),7]
  temp[1,4] <- out1[nrow(out),6] + out1[nrow(out),7]
  temp[1,5] <- (out[nrow(out),6] + out[nrow(out),7]) - (out1[nrow(out),6] + out1[nrow(out),7])
  print(temp[1,5])
  print(i)
  tauchange <- rbind.data.frame(tauchange, temp)
}

colnames(tauchange)[1:5] <- c("BetaAA","BetaHA","ICombTau0","ICombTau1","ChangeIComb")
plot(tauchange$BetaHA, tauchange$ChangeIComb, type = "l", xlab = "BetaHA",
     ylab = "CHange in IComb (Tau = 1 -> Tau = 0)")


tauchangeplot <- tauchange[,c(1,2,5)]

mat <- spread(tauchangeplot, key = "BetaAA", value = "ChangeIComb")
row.names(mat) <- mat$BetaHA
mat$BetaHA <- NULL
mat1 <- data.matrix(mat)

p5 <- plot_ly(z = mat1, x = betaHArange, y = betaAArange) %>% add_surface(
  cmin = 0, cmax = 1
) %>% layout(
  title = "Comb Equilibrium Prevalence of I<sub>H</sub>*",
  scene = list(
    camera = list(eye = list(x = -1.25, y = 1.25, z = 0.5)),
    xaxis = list(title = "betaAArange", nticks = 8, range = c(0,1)),
    yaxis = list(title = "betaHArange", nticks = 8, range = c(0,1)),
    zaxis = list(title = 'Chnage in IComb*', nticks = 8, range = c(-0.00001,1)),
    aspectratio=list(x=0.8,y=0.8,z=0.8)))
p5 

p6 <- plot_ly(x = betaAArange, y = betaHArange, z = mat1, type = "contour", transpose = TRUE,
              contours = list(start = -0.0001, end = 1, size = 0.05),
              colorbar = list(title = "I<sub>RH</sub>* + I<sub>H</sub>*")) %>% 
  layout(title = "Comb Equilibrium Prevalence of I<sub>H</sub>*",
         xaxis = list(title = "BetaAA"),
         yaxis = list(title = "BetaHA"))
p6
