...
lm.upp <- c(0.5, 1, 150, 1, 0.5, 0.0015)
...
epsilon_AMR <- c(0.3108*1, 0.3108*0.75, 0.3108*0.5, 0.3108*0.25, 0.3108*0.2, 0.3108*0.15, 0.3108*0.1, 0.3108*0.075, 0.3108*0.065, 0.3108*0.05)
...
#### Test Data ####
data1 <- cbind(read.csv("TEST_INC_RESULTS_1.csv", header = TRUE), "group" = "data1")
data2 <- cbind(read.csv("TEST_INC_RESULTS_2.csv", header = TRUE), "group" = "data2")
data3 <- cbind(read.csv("TEST_INC_RESULTS_3.csv", header = TRUE), "group" = "data3")
data4 <- cbind(read.csv("TEST_INC_RESULTS_4.csv", header = TRUE), "group" = "data4") 
data5 <- cbind(read.csv("TEST_INC_RESULTS_5.csv", header = TRUE), "group" = "data5") 
data6 <- cbind(read.csv("TEST_INC_RESULTS_6.csv", header = TRUE), "group" = "data6")
data7 <- cbind(read.csv("TEST_INC_RESULTS_7.csv", header = TRUE), "group" = "data7")
data8 <- cbind(read.csv("TEST_INC_RESULTS_8.csv", header = TRUE), "group" = "data8")
data9 <- cbind(read.csv("TEST_INC_RESULTS_9.csv", header = TRUE), "group" = "data9")
data10 <- cbind(read.csv("TEST_INC_RESULTS_10.csv", header = TRUE), "group" = "data10")


plot(density(data8$betaAA))
plot(density(data8$phi))
plot(density(data8$kappa))
plot(density(data8$alpha))
plot(density(data8$zeta))
plot(density(data8$betaHA))

map_phi <- map_estimate(data8[,"phi"], precision = 20) 
map_kappa <- map_estimate(data8[,"kappa"], precision = 20) 
map_betaAA <- map_estimate(data8[,"betaAA"], precision = 20) 
map_alpha <- map_estimate(data8[,"alpha"], precision = 20) 
map_zeta <- map_estimate(data8[,"zeta"], precision = 20) 
map_betaHA <- map_estimate(data8[,"betaHA"], precision = 20) 
...
output1 <- data.frame(matrix(0, nrow = length(parmtau), ncol=6))
...
for (i in 1:length(parmtau)) {
  ...
  parms2 = c(ra = 60^-1, rh =  (5.5^-1), ua = 240^-1, uh = 28835^-1, betaAA = map_betaAA, betaAH = 0.00001, betaHH = 0.00001, 
             betaHA = (map_betaHA), phi = map_phi, kappa = map_kappa, alpha = map_alpha, tau = parmtau[i], zeta = map_zeta)
  ...
  output1[i,] <- c(parmtau[i], 
                ...
                ((out[[2]] + out[[3]])*(446000000))/100000,
                out[[1]][["Ish"]] / (out[[1]][["Ish"]] + out[[1]][["Irh"]]),
                out[[1]][["Isa"]] / (out[[1]][["Isa"]] + out[[1]][["Ira"]]))
}

colnames(output1)[1:6] <- c("tau", "InfHumans","ResInfHumans","ICombH","IResRat","IResRatA")
...
plot_ly(output2, x= ~tau, y = ~InfHumans, type = "bar", name = "Antibiotic-Sensitive Infection (Human)") %>%
  ...
  layout(yaxis = list(title = "Proportion of Infected Humans (ICombH) per 100,000", range = c(0,2), showline = TRUE),
