...
outdata$Sh10 <- outdata$Sh*100000
...
#Human Population 
ggplot(outdata, aes(time)) +
  geom_line(aes(y = Sh10, colour = "Sh"), size = 1.1) +
...
parmsold = fast_parameters(minimum = c(2.5^-1,0.6^-1,24^-1,2883.5^-1,0,0,0,0,0,0,0), maximum = c(250^-1,60^-1,2400^-1,288350^-1,0.9,0.0001,0,0.001,0.1,0.1,5), 
                        factor=11, names = c("ra", "rh" ,"ua", "uh", "betaAA", "betaAH", "betaHH", "betaHA",
                                             "phi", "tau", "theta"))

parms = fast_parameters(minimum = c(0,0.7^-1,4.2^-1,2883.5^-1,0,0,0,0,0,0,0), maximum = c(420^-1,70^-1,420^-1,288350^-1,0.415,0.0001,0,0.001,0.1,0.1,5), 
                        factor=11, names = c("ra", "rh" ,"ua", "uh", "betaAA", "betaAH", "betaHH", "betaHA",
                                             "phi", "tau", "theta"))
...
for (i in 1:nrow(parms)) {
  ...
  out <- ode(y = init, func = amrold, times = times, parms = parms1)
