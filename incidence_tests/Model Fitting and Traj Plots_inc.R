...
rm(list=ls())
setwd("C:/Users/amorg/Documents/PhD/Chapter_2/Chapter2_Fit_Data/FinalData/NewFit/Remote_Fit")
...
dataamp <- read.csv("C:/Users/amorg/Documents/PhD/Chapter_2/Chapter2_Fit_Data/FinalData/NewFit/resistanceprofAnim_amp.csv")
...
datatetra <- read.csv("C:/Users/amorg/Documents/PhD/Chapter_2/Chapter2_Fit_Data/FinalData/NewFit/resistanceprofAnim_v1.csv")
...
databroil <- read.csv("C:/Users/amorg/Documents/PhD/Chapter_2/Chapter2_Fit_Data/FinalData/NewFit/salm_broilers_2018.csv")
...
MAPtet <- c(map_estimate(data[which(data$group == "data10" & data$fit == "tet"),][-c(6:7)])[,2]); MAPtet <-  MAPtet[order(c(3,1,2,4,5))]
MAPamp <- c(map_estimate(data[which(data$group == "data10" & data$fit == "amp"),][-c(6:7)])[,2]);  MAPamp <-  MAPamp[order(c(3,1,2,4,5))]
MAPbroil <- c(map_estimate(data[which(data$group == "data10" & data$fit == "broil"),][-c(6:7)])[,2]);   MAPbroil <- MAPbroil[order(c(3,1,2,4,5))]
  ...
  for (i in 1:length(colnames(MAP))) { # Loop over loop.vector
    ...
    plotlist2[[i]] <- local({
      ...
      if(colnames(MAP)[i] == "phi") {
        p1 <- p1 + scale_x_continuous(limits = c(0,0.5), expand = c(0, 0), name = expression(paste("Rate of Resistance Reversion (", phi, ")"))) +
      ...
      p1 <- p1 + geom_vline(xintercept = list(MAPtet, MAPamp, MAPbroil)[[j]][i], size  = 1.2, color = "red", alpha = 0.5)
...
ggsave(abc, filename = "ABC_SMC_Post_v2.png", dpi = 300, type = "cairo", width = 10, height = 10, units = "in",
