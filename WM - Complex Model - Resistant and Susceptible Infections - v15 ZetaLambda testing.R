...
ggplot(outdata, aes(time)) +
  geom_line(aes(y = Ih10, colour = "Ih"), size = 2.5, alpha =0.6) + 
  geom_line(aes(y = Irh10, colour = "Irh"), size = 1.1) +
  geom_line(aes(y = combIh10, colour = "combIh"), size = 1.1, linetype = "dashed") +
...
for (i in 1:length(parmtau)) {
  temp <- data.frame(matrix(NA, nrow = 1, ncol=5))
  parms2 = c(ra = 25^-1, rh =  (6^-1), ua = 240^-1, uh = 28835^-1, betaAA = (0.09), betaAH = 0.00001, betaHH = 0.00001, 
             betaHA = (0.00001), phi = 0.05, tau = parmtau[i], theta = 0.5, zeta = zetabase, lambda = 0.4)
  out <- ode(y = init, func = amr, times = times, parms = parms2)
  temp[1,1] <- parmtau[i]
  temp[1,2] <- rounding(out[nrow(out),5]) 
  temp[1,3] <- rounding(out[nrow(out),6]) 
...
betaHArange <- seq(0.000005,0.00001, by=0.0000001)
betaAArange <- seq(0.045,0.09, by = 0.0009)
betaaaperc <- seq(50, 101, by = 1)
betahaperc <- seq(50,101, by = 1)
#betaHArange <- seq(0,0.0001, by=0.00001)
...
for (i in 1:nrow(combparm1)) {
  temp <- data.frame(matrix(NA, nrow = 1, ncol=5))
  parms1 = c(ra = 25^-1, rh =  (6^-1), ua = 240^-1, uh = 28835^-1, betaAA = combparm1[i,2], betaAH = 0.00001, betaHH = 0.00001, 
             betaHA = combparm1[i,1], phi = 0.05, tau = 0, theta = 0.5, zeta = zetabase, lambda = 1)
  out <- ode(y = init, func = amr, times = times, parms = parms1)
  print(out[nrow(out),7])
  temp[1,1] <- combparm1[i,1]
  temp[1,2] <- combparm1[i,2]
...
p100 <- plot_ly(x = betaaaperc, y = betahaperc, z = mat3, type = "contour",
               contours = list(start = 1.389, end = 3.05501, size = 0.2, showlabels = TRUE,
                               labelfont = list(size = 12, color = 'white')),
               colorbar = list(title = "I<sub>CombH</sub>", 
                               thickness = 35, len = 1)) %>% 
...
parmtau <- seq(0,0.2,by=0.005)
#parmtau <- seq(0,0.05,by=0.005)

init <- c(Sa=0.99, Ia=0.01, Ira=0, Sh=1, Ih=0, Irh=0)
outputlambda04 <- data.frame()
times <- seq(0, 200000, by = 10)

for (i in 1:length(parmtau)) {
  temp <- data.frame(matrix(NA, nrow = 1, ncol=5))
  parms2 = c(ra = 25^-1, rh =  (6^-1), ua = 240^-1, uh = 28835^-1, betaAA = (0.09), betaAH = 0.00001, betaHH = 0.00001, 
             betaHA = (0.00001), phi = 0.05, tau = parmtau[i], theta = 0.5, zeta = 1, lambda = 0.4)
  out <- ode(y = init, func = amr, times = times, parms = parms2)
  temp[1,1] <- parmtau[i]
  temp[1,2] <- rounding(out[nrow(out),5]) 
  temp[1,3] <- rounding(out[nrow(out),6]) 
  ...
  temp[1,6] <- temp[1,4]/temp[1,5]
  print(temp[1,3])
  outputlambda04 <- rbind.data.frame(outputlambda04, temp)
}

colnames(outputlambda04)[1:6] <- c("tau", "SuscHumans","InfHumans","ResInfHumans","ICombH","IResRat")
...
outputlambda04$InfHumans <- outputlambda04$InfHumans*100000
outputlambda04$ResInfHumans <- outputlambda04$ResInfHumans*100000
outputlambda04$ICombH <- outputlambda04$ICombH*100000
...
ggplot(data = new.data1, aes(x = tau, y = IResRat, colour = state)) +
  ...
  scale_x_continuous(limits = c(0,0.2), expand = c(0,0)) +
  scale_y_continuous(limits = c(0,1), expand = c(0,0)) +
  scale_colour_manual(values = c("steelblue1","blue","slateblue1","slategray","black"), 
                      labels = c(expression(paste(lambda," = 60%")),
...
ggplot() + 
  ...
  geom_line(data=outputlambda09, aes(tau,ICombH), size = 1.1, colour = "slategray") + 
  geom_line(data=outputlambda08, aes(tau,ICombH), size = 1.1, colour = "slateblue1") + 
  geom_line(data=outputlambda07, aes(tau,ICombH), size = 1.1, colour = "blue") +
  geom_line(data=outputlambda06, aes(tau,ICombH), size = 1.1, colour = "steelblue1") +
  scale_x_continuous(limits = c(0,0.2), expand = c(0,0)) +
