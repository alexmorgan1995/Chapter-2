...
p_amp_pigs <- p_amp_pigs + geom_line(aes(y = predict(stat_amp_pigs, melt_amp_pigs), x = usage), 
                                 colour = "red", size = 1.2) +
  annotate("text",label = paste0("p-value: ", round(summary(stat_amp_pigs)[[4]][[8]], digits = 6)), y = 0.95, 
           x = max(melt_amp_pigs$usage, na.rm = TRUE)*0.75, size = 5)

p_tet_pigs <- p_tet_pigs + geom_line(aes(y = predict(stat_tet_pigs, melt_tet_pigs), x = usage), 
                                 colour = "red", size = 1.2) +
  annotate("text",label = paste0("p-value: ", round(summary(stat_tet_pigs)[[4]][[8]], digits = 6)), y = 0.95, 
           x = max(melt_tet_pigs$usage, na.rm = TRUE)*0.75, size = 5)

p_amp_broil <- p_amp_broil + geom_line(aes(y = predict(stat_amp_broil, melt_amp_broil), x = usage), 
                                   colour = "red", size = 1.2) +
  annotate("text",label = paste0("p-value: ", round(summary(stat_amp_broil)[[4]][[8]], digits = 6)), y = 0.95, 
           x = max(melt_amp_broil$usage, na.rm = TRUE)*0.75, size = 5)

p_tet_broil <- p_tet_broil + geom_line(aes(y = predict(stat_tet_broil, melt_tet_broil), x = usage), 
                                  colour = "red", size = 1.2) +
  annotate("text",label = paste0("p-value: ", round(summary(stat_tet_broil)[[4]][[8]], digits = 6)), y = 0.95, 
