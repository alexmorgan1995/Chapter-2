...
ABC_algorithm <- function(N, G, sum.stats, distanceABC, fitmodel, tau_range, init.state, data)  {
  ...
  for(g in 1:G) {
    ...
    while(i <= N) {
      ...
      if(g==1) {
        d_betaAA <- runif(1, min = 0, max = 0.4)
        d_phi <- runif(1, min = 0, max = 0.3)
    ...
    write.csv(res.new, file = paste("ABC_post_tetpigs_",g,".csv",sep=""), row.names=FALSE)
  }
  return(N_ITER_list)
}
...
lm.upp <- c(0.4, 0.3, 20, 1, 1, 0.002) #Upper and lower bounds for the priors - for the multivariate normal dist pert kernel
...
#Thresholds 
epsilon_dist <- c(4, 3.5, 3, 2.5, 2.25, 2, 1.8, 1.7, 1.6, 1.5)
