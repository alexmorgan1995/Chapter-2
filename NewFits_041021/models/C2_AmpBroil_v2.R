...
ABC_algorithm <- function(N, G, sum.stats, distanceABC, fitmodel, tau_range, init.state, data)  {
  ...
  for(g in 1:G) {
    ...
    write.csv(res.new, file = paste("ABC_post_ampbroil_",g,".csv",sep=""), row.names=FALSE)
  }
  return(N_ITER_list)
}
...
lm.upp <- c(1, 0.5, 40, 1, 2, 0.002) #Upper and lower bounds for the priors - for the multivariate normal dist pert kernel
...
#Thresholds 
epsilon_dist <- c(6, 5, 4, 3.5, 3, 2.75, 2.5, 2.4, 2.3, 2.2)
