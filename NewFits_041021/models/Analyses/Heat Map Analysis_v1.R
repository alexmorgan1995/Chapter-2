...
library("grid"); library("gridExtra"); library("rootSolve")
...
for(j in 1:4) {
  ...
  heatmap[[j]] = local({
    ...
    for(z in 1:3) {
      ...
      if(z == 1) {
        parameterspace <- expand.grid("betaAA" = seq(parms[["betaAA"]]*0.5, parms[["betaAA"]], by = (parms[["betaAA"]]*0.5)/50), 
                                      "betaHA" = seq(parms[["betaHA"]]*0.5, parms[["betaHA"]], by = parms[["betaHA"]]*0.5/50))  
      }
      if(z == 2) {
        parameterspace <- expand.grid("zeta" = seq(parms[["zeta"]]*0.5, parms[["zeta"]], by = (parms[["zeta"]]*0.5)/50), 
                                      "betaHA" = seq(parms[["betaHA"]]*0.5, parms[["betaHA"]], by = parms[["betaHA"]]*0.5/50))
      }
      if(z == 3) {
        parameterspace <- expand.grid("both" = seq(0.5, 1, by = 0.01), 
                                      "betaHA" = seq(parms[["betaHA"]]*0.5, parms[["betaHA"]], by = parms[["betaHA"]]*0.5/50))
      }
...
combplot <- ggarrange(plotheat[[1]], plotheat[[2]], plotheat[[3]], ncol = 3, nrow = 1)
...
scentest3 <- heatmap[[4]][[3]]

breaks1 <- c(0, 0.593, seq(0.593, max(scentest1$icombh)+0.1, by = 0.1))

plot1 <- ggplot(scentest1, aes(percdecrease, percbetaHA, z = icombh)) + metR::geom_contour_fill(breaks = breaks1, color = "black", size = 0.1)  + 
  labs(x = bquote("% of Baseline"~beta["AA"]~"and"~ zeta), y = bquote("% of Baseline"~beta["HA"]), fill = "ICombH", title = "Ampicillin Usage in Broiler Poultry") +
  geom_contour(color = "red", size = 1, breaks = 0.593, alpha = 0.8) +
...
breaks2 <- c(0, 0.593, seq(0.593, max(scentest2$icombh)+0.1, by = 0.1))
...
breaks3 <- c(0, 0.593, seq(0.593, max(scentest3$icombh)+0.1, by = 0.1))

plot3 <- ggplot(scentest3, aes(percdecrease, percbetaHA, z = icombh)) + metR::geom_contour_fill(breaks = breaks3, color = "black", size = 0.1)  + 
  labs(x = bquote("% of Baseline"~beta["AA"]~"and"~ zeta), y = bquote("% of Baseline"~beta["HA"]), fill = "ICombH", title = "Ampicillin Usage in Fattening Pigs") +
  geom_contour(color = "red", size = 1, breaks = 0.593, alpha = 0.8) +
...
breaks4 <- c(0, 0.593, seq(0.593, max(scentest4$icombh)+0.1, by = 0.1))
...
combplot_solo <- ggarrange(plot1, plot2, plot3,  plot4, ncol = 1, nrow = 4, common.legend = TRUE,
                      legend = "bottom", labels = c("A", "B", "C", "D"), font.label = list(size = 25), vjust = 1.2)
