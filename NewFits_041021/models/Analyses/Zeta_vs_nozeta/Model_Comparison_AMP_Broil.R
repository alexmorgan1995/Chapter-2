...
ABC_algorithm <- function(N, G, sum.stats, distanceABC, fitmodel, tau_range, init.state, data)  {
  ...
  for(g in 1:G) {
    ...
    write.csv(res.new, file = paste("//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_2/Models/Chapter-2/NewFits_041021/data/new/compare/COMPARE_ABC_post_ampbroil_",g,".csv",sep=""), row.names=FALSE)
  }
  return(N_ITER_list)
}
