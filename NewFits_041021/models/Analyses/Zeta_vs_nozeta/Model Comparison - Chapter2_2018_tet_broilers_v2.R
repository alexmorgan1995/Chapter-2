...
#### Test Data ####
post_dist_m <- list()
post_dist_m[[1]] <- cbind(read.csv("COMP_results_ABC_SMC_gen_broil_1.csv", header = TRUE), "group" = "data1")
post_dist_m[[2]]  <- cbind(read.csv("COMP_results_ABC_SMC_gen_broil_2.csv", header = TRUE), "group" = "data2")
post_dist_m[[3]]  <- cbind(read.csv("COMP_results_ABC_SMC_gen_broil_3.csv", header = TRUE), "group" = "data3")
post_dist_m[[4]]  <- cbind(read.csv("COMP_results_ABC_SMC_gen_broil_4.csv", header = TRUE), "group" = "data4") 
...
post_dist_m_data_melt$scen <- "tet_broil"

write.csv(post_dist_m_data_melt, file = "comp_tet_broil.csv")
...
ggplot(post_dist_m_data_melt, aes(x = gen, y = value, fill = as.factor(variable))) + geom_bar(position = "fill", stat="identity") + 
  ...
  labs(title = "Model Comparison (Tetracycline usage in Broiler Poultry)")
