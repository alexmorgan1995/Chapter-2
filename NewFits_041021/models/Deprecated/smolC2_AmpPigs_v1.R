...
ABC_algorithm <- function(N, G, sum.stats, distanceABC, fitmodel, tau_range, init.state, data)  {
  ...
  for(g in 1:G) {
    ...
    write.csv(res.new, file = paste("TESTABC_post_amppigs_",g,".csv",sep=""), row.names=FALSE)
  }
  return(N_ITER_list)
}
...
lm.upp <- c(0.3, 0.3, 5, 1, 0.5, 0.002) #Upper and lower bounds for the priors - for the multivariate normal dist pert kernel
