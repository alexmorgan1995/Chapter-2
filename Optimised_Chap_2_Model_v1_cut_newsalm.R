...
computeDistanceABC_ALEX <- function(sum.stats, distanceABC, fitmodel, tau_range, thetaparm, init.state, times, data) {
  ...
  return(c(distanceABC(list(sum.stats), data, tauoutput[!tauoutput$tau == 0.0106,]),
           ...
           abs(tauoutput$ResPropHum[tauoutput$tau == 0.0062] - 0.28)))
}
...
epsilon_dist <- c(4, 3.5, 3, 2.5, 2)
epsilon_food <- c(3.26*0.3, 3.26*0.25, 3.26*0.2, 3.26*0.15, 3.26*0.12)
epsilon_AMR <- c(0.28*0.3, 0.28*0.25, 0.28*0.2, 0.28*0.15,  0.28*0.12)
